...
# service. devaJ's own IAM grant only allows it to create roles named
# "devaJ-*", and only with this account's permissions boundary attached
# (a privilege-escalation guardrail) - hence the role name and
# --permissions-boundary flag below. If your account has no such
# constraint, drop both and use any role name you like.
#
...
ROLE_NAME="devaJ-j7website-short-url-github-deploy"
PERMISSIONS_BOUNDARY_ARN="arn:aws:iam::857475015624:policy/boundary-devaJ"
OIDC_URL="https://token.actions.githubusercontent.com"

echo "==> devaJ's grant doesn't include iam:*OpenIDConnectProvider* actions, so we"
...
aws iam put-role-policy \
  --role-name "$ROLE_NAME" \
  --policy-name devaJ-j7website-short-url-deploy \
